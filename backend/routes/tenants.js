...
router.get('/check-id/:idNumber', async (req, res) => {
  try {
    ...
    // Query for tenant with this ID number
    const tenantResult = await db.query(
      `SELECT id, name, nationality, country_code, contact_number, email, id_document_path
       FROM tenants
       WHERE id_number = $1`,
      [idNumber]
    );
    ...
    // Get the last rental agreement details
    const rentalResult = await db.query(
      `SELECT ra.id, ra.contract_number, ra.start_date, ra.end_date, ra.rental_amount,
              ra.balance_amount, ra.is_active,
    ...
    const response = {
      ...
      tenant: {
        ...
        contact: tenant.country_code ? `${tenant.country_code} ${tenant.contact_number}` : tenant.contact_number,
        email: tenant.email,
    ...
    if (rentalResult.rows.length > 0) {
      ...
      response.last_rental = {
        building: rental.building_name,
        flat: rental.flat_number,
      ...
      response.current_status = rental.is_active ? 'active' : 'inactive';
    } else {
      response.current_status = 'inactive';
    }

    res.json(response);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to check tenant ID' });
  }
});
